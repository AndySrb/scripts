...
parser = argparse.ArgumentParser(description='Convert between hexadecimal,binary and decimal values')
parser.add_argument('-c','--convert', dest='convert', action='store',
choices={'db','dh','bd','bh','hd','hb'},
help='d= decimal b=binary h=hexadecimal first letter is form which format we are converting from seccond letter is desired format example: py-hexbindec.py -c dh -i 25 this will convert form decimal to hexadecimal')
